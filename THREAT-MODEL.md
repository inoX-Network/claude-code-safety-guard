...
- **Fail-closed** — if the rules file is missing, unreadable, or empty, a
  hardcoded minimal ruleset takes over instead of allowing everything. The same
  applies to the run itself: any unexpected error during a check, and any input
  the hook cannot parse, ends in a denial. Only exit code 2 means "deny", so an
  uncaught error would otherwise be an unnoticeable way past the guard — the
  command runs and it looks like a normal allow.
...
| **A hook file that cannot even be loaded.** The safety net that turns crashes into denials lives *inside* `command-guard.py`. If the file itself fails to import — a syntax error from an edit, a missing dependency — Python aborts before any of it runs, exits `1`, and the call is allowed. | Code cannot catch an error that happens while that same code is being parsed. | **A wrapper outside the file** — point the hook setting at a small launcher that runs the guard and maps every exit code other than `0` and `2` onto `2`. Plus: never edit a live hook without running the suites first. |
